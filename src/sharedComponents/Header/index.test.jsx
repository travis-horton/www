...
test('marks the correct link as selected on a sub-route', () => {
  const { getByText } = render(
    <MemoryRouter
      future={{ v7_startTransition: true, v7_relativeSplatPath: true }}
      initialEntries={['/programming']}
    >
      <Header />
    </MemoryRouter>,
  );
  expect(getByText('software engineer').closest('a')).toHaveClass(
    'nav__item--selected',
  );
});
...
test('does not mark other links as selected', () => {
  ...
  expect(getByText('about me').closest('a')).not.toHaveClass(
    'nav__item--selected',
  );
  expect(getByText('pianist').closest('a')).not.toHaveClass(
    'nav__item--selected',
  );
});
