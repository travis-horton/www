...
const Digit = ({
  ...
  return (
    <line
      ...
      stroke="black"
      strokeWidth={SCALE * 0.75}
    />
  );
};

const Hex = ({ value, startVec, showMoreSigBits }) => {
  return (
    <>
...
const BinaryNumber = ({ startVec, value }) => {
  const hexDigits = value
    ? Math.ceil((1 + Math.floor(Math.log2(value))) / 4)
    : 1;
  ...
  return (
    <>
      {[...Array(hexDigits).keys()].map((idx) => {
        ...
        const startX =
          thisHexStartX + (hideMoreSigBits ? SCALE * (3 - digits) : SCALE);
        const endX = thisHexStartX + SCALE * 3;
        const curveStartX =
          startX - SCALE + (hideMoreSigBits && digits ? SCALE : 0);
    ...
    <div>
      ...
      <svg viewBox={`0 0 ${SIZE} ${SIZE * 8}`}>
        ...
        {[...Array(16).keys()].map((idx) => (
          <>
            <text x={14} y={40 + 6 * SCALE + SCALE * (6 * idx)}>
              {idx}
            </text>
            <BinaryNumber
              startVec={{ x: 6, y: 42 + SCALE * (6 * idx) }}
              value={idx}
            />
          </>
        ))}
